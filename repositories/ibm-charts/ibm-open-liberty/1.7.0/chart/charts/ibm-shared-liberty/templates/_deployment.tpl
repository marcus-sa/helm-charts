...
{{ toYaml . | indent 6 }}
{{- end -}}
      {{- if or $root.Values.pod.extraInitContainers $root.Values.sessioncache.hazelcast.enabled }}
      initContainers:
      {{ if $root.Values.sessioncache.hazelcast.enabled }}
      - name: init-hazelcast
...
{{- end }}
        ...
        {{- end }}
        - name: JVM_ARGS
          value: "{{ if $root.Values.sessioncache.hazelcast.embedded }}-Dhazelcast.jcache.provider.type=server -DserviceName={{ include "slt.utils.servicename" (list $root) }}{{ end }} {{ $root.Values.env.jvmArgs }}"
