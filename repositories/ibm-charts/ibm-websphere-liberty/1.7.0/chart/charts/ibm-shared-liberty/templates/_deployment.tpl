...
{{ toYaml . | indent 4 }}
{{- end }}
  {{- if $root.Values.deployment.annotations }}
  annotations:
...
{{ toYaml . | indent 6 }}
{{- end -}}
      {{ if or $root.Values.pod.extraInitContainers $root.Values.sessioncache.hazelcast.enabled (and $root.Values.isOpenShift $stateful) }}
      initContainers:
      {{ if $root.Values.sessioncache.hazelcast.enabled }}
      - name: init-hazelcast
      ...
      {{ end }}
      {{ if $root.Values.isOpenShift }}
      {{ if $root.Values.logs.persistLogs }}
      - name: logs-dir-permissions
        image: busybox
        command: ['sh', '-c', 'chgrp -R 0 /logs && chmod -R g=u /logs']
        volumeMounts:
        - mountPath: /logs
          name: {{ $root.Values.persistence.name | trunc 63 | lower | trimSuffix "-" | quote }}
      {{ end }}
      {{ if $root.Values.logs.persistTransactionLogs }}
      - name: tranlog-dir-permissions
        image: busybox
        command: ['sh', '-c', 'chgrp -R 0 /output/tranlog && chmod -R g+rwX /output/tranlog']
        volumeMounts:
...
{{ toYaml $root.Values.pod.extraInitContainers | indent 6}}
      {{- end }}
      {{ end }}
      affinity:
      ...
      - name: {{ $root.Chart.Name }}
        ...
        lifecycle:
{{ toYaml $root.Values.image.lifecycle | indent 10 }}
        {{- end }}
        env:
...
{{ toYaml $root.Values.resources.requests | indent 12 }}
        ...
        securityContext:
          privileged:  false
      {{- if $root.Values.pod.extraContainers }}
{{ toYaml $root.Values.pod.extraContainers | indent 6}}
      {{- end }}
