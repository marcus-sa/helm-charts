...
values.yaml and values-metadata.yaml. Doing so will allow the customer to override the
default affinity values specified in _sch-chart-config.yaml.
...
nodeAffinity:
  ...
  {{ include "sch.affinity.nodeAffinityRequiredDuringScheduling" $root }}
    {{- end }}
    {{- if or (hasKey $nodeAffinity "nodeAffinityPreferredDuringScheduling") (hasKey $root.Values "arch") }}
  {{ include "sch.affinity.nodeAffinityPreferredDuringScheduling" $root }}
    {{- end }}
  {{- end }}
{{- end }}
...
{{- define "sch.affinity.nodeAffinityRequiredDuringScheduling" -}}
    {{- $root := . -}}
    {{- $affinity := $root.sch.chart.nodeAffinity.nodeAffinityRequiredDuringScheduling -}}
...
{{- define "sch.affinity.nodeAffinityPreferredDuringScheduling" -}}
  {{- $root := . -}}
  {{- $params := $root.sch.chart.nodeAffinity.nodeAffinityPreferredDuringScheduling -}}
  {{- $affinity := $root.Values.arch | default $params -}}
